...
class RiskControlSystem:
    ...
    def __init__(self, data_path: str, config: Dict = None):
        ...
        self.results = {}
        self.filters = {
            'ST股票': lambda row: not str(row.get('name', '')).startswith('ST'),
            '停牌股票': lambda row: row.get('is_suspended', 0) == 0,
            '退市股票': lambda row: not str(row.get('name', '')).startswith('*ST'),
        }
    ...
    def _generate_markdown_report(self) -> str:
        ...
        # 风险控制摘要
        if self.control_summary:
            lines.append("## 🛡️ 风险控制摘要\n")
            lines.append(self.format_control_summary(self.control_summary))
            lines.append("\n")
