...
class MarketStateIdentifier:
    ...
    def __init__(self):
        """初始化市场状态识别器"""
        self.manager = MarketStateManager()
        self._is_trained = False
    
    def identify_current_state(self, index_data: pd.DataFrame = None) -> str:
        """
        识别当前市场状态
        
        Args:
            index_data: 指数数据，如果为None则返回默认状态
            
        Returns:
            市场状态字符串
        """
        if index_data is None or not self._is_trained:
            return "震荡市"
        
        try:
            result = self.manager.update(index_data)
            return result['market_state']
        except Exception:
            return "震荡市"
    
    def train(self, historical_data: pd.DataFrame, sector_data: pd.DataFrame = None):
        """
        训练市场状态分类器
        ...
        Args:
            ...
            sector_data: 历史行业数据（可选）
        """
        self.manager.train(historical_data, sector_data)
        self._is_trained = True
    
    def get_strategy_weights(self, index_data: pd.DataFrame = None) -> Dict[str, float]:
        """
        获取当前市场状态下的最优策略权重
        ...
        try:
            result = self.manager.update(index_data)
            return result['optimal_strategy_weights']
        except Exception:
            return {}
