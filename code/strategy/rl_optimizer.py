...
class RLOptimizer:
    ...
    def __init__(self, market_data: pd.DataFrame = None, strategy_returns: pd.DataFrame = None):
        """
        初始化强化学习优化器
    ...
    def train(self, market_data: pd.DataFrame = None, strategy_returns: pd.DataFrame = None, 
        ...
        if self.market_data is None or self.strategy_returns is None:
            return {'success': False, 'message': '缺少训练数据'}
        
        try:
            self.portfolio_manager = RLPortfolioManager(self.market_data, self.strategy_returns)
            rewards, portfolio_values = self.portfolio_manager.train(episodes=episodes)
    ...
    def predict(self, state=None) -> np.ndarray:
        ...
        Returns:
            最优权重数组
        """
        if not self._is_trained or self.portfolio_manager is None:
            if self.strategy_returns is not None:
                n_strategies = self.strategy_returns.shape[1]
                return np.ones(n_strategies) / n_strategies
            return np.array([0.5, 0.5])
        
        return self.portfolio_manager.get_optimal_weights(state)
