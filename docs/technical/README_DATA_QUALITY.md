...
```python
from code.data.data_quality_framework import DataQualityPipeline
import pickle
...
```python
from code.data.data_quality_framework import DataQualityChecker, DataCleaner
...
```python
from code.data.data_quality_framework import DataQualityChecker
...
# 运行核心框架（演示模式）
python3 code/data/data_quality_framework.py
```
...
```python
# 获取数据后立即进行质量检查
from code.data.fetch_real_data import RealAStockDataFetcher
from code.data.data_quality_framework import DataQualityPipeline
...
- 代码实现：`code/data/data_quality_framework.py`
